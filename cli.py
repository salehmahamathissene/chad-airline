...
def main() -> int:
    ...
    # ---------------------------
    # PRO DEFAULT SUBCOMMAND FIX
    # ---------------------------
    # Docker runs: `python -m cli` (no subcommand)
    # Argparse will NOT create args.flights unless "run" is parsed.
    # So we inject "run" BEFORE parsing when no subcommand was provided.
    argv = sys.argv[1:]
    if len(argv) == 0 or argv[0].startswith("-"):
        argv = ["run"] + argv

    args = parser.parse_args(argv)
    cmd = args.cmd  # always set now
    # ---------------------------
